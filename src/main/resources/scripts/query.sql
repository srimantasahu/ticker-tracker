...
-- microcap: pe<5 and sect_index is null or NA, order by ltp
...
and (sect_index is null or sect_index = 'NA') 
...
-- smallcap: pe<10 and +eps, order by ltp - low_52w

select sect_index, symbol, ltp, high_52w, low_52w, adjusted_pe, symbol_pe, earnings_share, promoter_holding, public_holding, per_chng_365d, per_chng_30d, tot_mar_cap_cr, face_val, price_band, buy_qty, sell_qty from stocks.refdata 
where 1=1
and (symbol_pe is null or symbol_pe < 10) 
...
-- midcap: pe<15 and +eps, order by eps
...
-- largecap: pe<20 and +eps, order by 365d change
